...
void * thread_pixels( void *thread_num)
{
    int col = *(int *) thread_num;
    Vec3 camera_pos;
...
main( int argc, char **argv )
{
    ...
    int * threadID = malloc(sizeof(int) * nthreads);
    int num, i;

    for(i = 0; i < nthreads; i ++)
    {
        threadID[i] = i;
        num = pthread_create(&threads[i], NULL, thread_pixels, (void *)(threadID + i));
        if (num)
        {
    ...
    for(i = 0; i < nthreads; i ++)
    {
        num = pthread_join(threads[i], NULL);
        if(num)
        {
